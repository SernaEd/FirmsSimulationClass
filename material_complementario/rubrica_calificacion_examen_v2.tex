% ============================================================
%  RÚBRICA DE CALIFICACIÓN — PRIMER EXAMEN PARCIAL — VERSIÓN B
%  Cálculo III: Ecuaciones Diferenciales en Ingeniería (MT024)
%  Universidad Iberoamericana
%
%  NOTA PARA EL PROFESOR (comentario LaTeX, no se imprime):
%  Documento hermano de examen_primer_parcial_v2.tex y de su
%  solucionario (examen_primer_parcial_v2_solucionario.tex). El
...
%  lista de cotejo, igual que la Versión A.
% ============================================================
...
% ---------- Encabezado ----------
\begin{center}
{\Large\bfseries RÚBRICA DE CALIFICACIÓN — PRIMER EXAMEN PARCIAL --- Versión B}\\[2pt]
{\small Cálculo III: Ecuaciones Diferenciales en Ingeniería (MT024) · Universidad Iberoamericana}\\[1pt]
{\small Documento hermano de \textit{examen\_primer\_parcial\_v2.tex} y su solucionario · Uso exclusivo del profesor}
\end{center}
...
b) & Tipo y orden correctos (\emph{orden} = 2; no confundir con el grado del término, que es 4) & 1 & \\
...
d) & Linealidad justificada (identifica el producto $r\,r'$ o el coeficiente dependiente de $r$) & 1.5 & \\
\end{tablarubrica}
...
b) & Identifica el intervalo abierto $(-1,1)$ con su justificación & 1 & \\
...
b) & Factor integrante $\mu(x)=x^{-2}$ correcto & 3 & \\
...
c) & Cita el teorema del caso lineal explícitamente para justificar $I=(-\infty,0)$ (no basta con repetir el intervalo de (a) sin conectar por qué es también el de la solución) & 4 & \\
\end{tablarubrica}
...
\textit{Nota: si el alumno prueba $\mu(y)$ primero y llega a un callejón sin salida, puede recibir crédito parcial en (b) por el intento correcto de verificación, pero 0 pts en (c) si no logra una ecuación exacta.}
...
\begin{tablarubrica}
a) & Formular: signo correcto ($-0.4\sqrt h$, no $+0.4\sqrt h$); 0 pts si el signo está mal sin justificar & 3 & \\
...
d) & Aplica la condición inicial ($h(0)=25$) & 2 & \\
d) & Encuentra $t=25$ con su restricción de dominio ($0\le t\le25$) & 1 & \\
e) & \textbf{Restringe el dominio de la fórmula y explica qué pasa físicamente para $t>25$} — punto central del problema & 5 & \\
\end{tablarubrica}

\textit{Nota crítica en (e): no dar crédito completo si el alumno solo reporta $h(t)=(5-0.2t)^2$ sin restringir el dominio ni explicar la continuación física, incluso si el resto del problema está correcto.}
...
\begin{tablarubrica}
a) & Identifica que $y=3$ es una solución constante perdida al dividir entre $h(y)=y^2-9$ & 3 & \\
a) & Explica correctamente por qué ``aproximar'' el logaritmo es matemáticamente inválido en un PVI & 2 & \\
b) & Verifica que $y\equiv3$ satisface la ecuación y la condición inicial & 4 & \\
...
b) & Conclusión final clara ($y(x)=3$ para todo $x$) & 2 & \\
\end{tablarubrica}

\textit{Notas: en (a), 0 pts si el alumno solo señala un error aritmético sin mencionar la solución constante perdida. En (b), máximo 6/10 si el alumno da $y=3$ sin justificar la unicidad con el teorema.}
