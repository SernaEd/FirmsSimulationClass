...
\begin{enumerate}[label=\textbf{\alph*)}, itemsep=8pt]
  \item $x^2 y''' + 2x y' - 9y = e^{-x}$: \textbf{orden 3} (la derivada de mayor orden que aparece es $y'''$). \textbf{Lineal}: $y$, $y'$, $y'''$ aparecen a la primera potencia y los coeficientes ($x^2$, $2x$, $-9$) dependen solo de la variable independiente $x$.
  \item $(y'')^5 - x^2y' + 3y = \sin x$: \textbf{orden 2} (la derivada de mayor orden presente es $y''$; el exponente 5 es el \emph{grado} del término, no el orden de la ecuación — no deben confundirse). \textbf{No lineal}: $y''$ aparece elevada a la potencia 5, y la condición de linealidad exige que \emph{cada} derivada (no solo $y$) aparezca a la primera potencia.
  \item $u_{tt}-4u_{xx}=0$: \textbf{orden 2}. \textbf{Lineal} (ecuación de onda: todos los términos son derivadas de $u$ a la primera potencia con coeficientes constantes).
  \item $q\,p'' + p\,p' - 4p = q^2$: \textbf{orden 2}. \textbf{No lineal}: el término $p\,p'$ es un producto de la variable dependiente por su propia derivada; además el \emph{coeficiente} de $p'$ es $p$ mismo, y la condición de linealidad exige que los coeficientes dependan únicamente de la variable independiente $q$, nunca de $p$.
\end{enumerate}
...
\subparte{b) Clasificar}{Orden 1. \textbf{No lineal} (el término $\sqrt{h}$ no es lineal en $h$), pero sí es \textbf{separable}: $\dfrac{dh}{\sqrt{h}} = -0.6\,dt$.}
