% ============================================================
%  EXAMEN — PRIMER EXAMEN PARCIAL — VERSIÓN B
...
%  - Documento hermano de examen_primer_parcial.tex (Versión A): misma
%    estructura de 6 problemas, mismo reparto de puntos por problema
%    (10/18/18/18/21/15 = 100 pts) y mismas técnicas evaluadas en cada
%    reactivo, para garantizar el mismo nivel de dificultad y el mismo
%    calibre de tiempo ("regla de 3x": profesor ~30 min, alumno ~90
%    min). Solo cambian los números, el contexto y, en algunos
%    incisos, la orientación de la técnica (p. ej. Problema 4 pide
%    mu(x) en vez de mu(y)) para que no sea una copia literal.
...
% ---------- Encabezado ----------
\begin{center}
{\Large\bfseries PRIMER EXAMEN PARCIAL --- Versión B}\\[2pt]
...
\begin{enumerate}[label=\textbf{\alph*)}, leftmargin=2em, itemsep=10pt]
  \item $\displaystyle x^3\,\frac{d^3y}{dx^3} + x\,\frac{dy}{dx} - 4y = \ln x$
  \item $\displaystyle \left(\frac{d^2y}{dx^2}\right)^{4} - x\,\frac{dy}{dx} + 6y = e^{2x}$
  \item $\displaystyle \frac{\partial^2 u}{\partial x^2} - \frac{\partial u}{\partial t} = 0$
  \item $\displaystyle \theta\, r'' + r\,r' - 3r = \theta^2 \qquad (r \text{ es función de } \theta)$
\end{enumerate}
...
Considera el PVI
\[
x\,\frac{dy}{dx} - 2y = x^3 e^{-x}, \qquad y(-1) = 2e.
\]
...
Un tanque esférico de almacenamiento en una planta procesadora de jugo se vacía por una válvula en la base. Estudios de mecánica de fluidos para este tanque indican que la razón de cambio de la altura del líquido $h(t)$ (en cm, con $t$ en minutos) es proporcional a la raíz cuadrada de la altura instantánea, con constante de proporcionalidad $k = 0.4 \text{ cm}^{1/2}/\text{min}$. Al iniciar la medición ($t=0$) el tanque contiene líquido hasta una altura de $25$ cm.
...
\begin{quote}
\small
``Separando variables: $\displaystyle\int \frac{dy}{y^2-9} = \int dx$. Por fracciones parciales, $\displaystyle \frac{1}{6}\ln\left|\frac{y-3}{y+3}\right| = x + C$. Al aplicar $y(0)=3$ no puedo evaluar el logaritmo porque el argumento es cero, así que tomo $y(0) \approx 3$ y obtengo $C \approx -9$ (un número muy grande y negativo), por lo que la solución aproximada es $\displaystyle\frac{1}{6}\ln\left|\frac{y-3}{y+3}\right| = x - 9$.''
\end{quote}
