...
def preview_attachment(
    ...
    PDF convertido de un PPT/PPTX (ver save_attachment). Mismos chequeos de
    visibilidad que /download — es un adjunto más, no un atajo sin auth."""
    attachment = get_attachment_or_404(db, attachment_id)
    if attachment.session_id != session_id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Adjunto no existe.")
    if not session_is_visible_to(user, attachment.session):
    ...
    if attachment.content_type == "application/pdf":
        preview_source = Path(attachment.storage_path)
    elif attachment.preview_path:
        preview_source = Path(attachment.preview_path)
    else:
        raise HTTPException(
